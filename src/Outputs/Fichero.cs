...
namespace DocumentPlagiarismChecker.Outputs
{
    ...
    /// </summary>
    internal class Fichero: Core.BaseOutput{
        /// <summary>
        ...
        public override void Write(List<FileMatchingScore> results, DisplayLevel level = DisplayLevel.BASIC){            
            foreach(FileMatchingScore fms in results){
                ...
                fichero.Write("  Arhchivo de la izquierda: ");
                Console.ForegroundColor = ConsoleColor.White;
                fichero.WriteLine(fms.LeftFileName);
                
                Console.ForegroundColor = ConsoleColor.DarkYellow;
                fichero.Write("  Archivo de la derecha: ");
                Console.ForegroundColor = ConsoleColor.White;
                fichero.WriteLine(fms.RightFileName);

                Console.ForegroundColor = ConsoleColor.DarkYellow;
                fichero.Write("  Matching: ");                
                Console.ForegroundColor = (fms.Matching < GetThreshold(DisplayLevel.BASIC) ? ConsoleColor.DarkGreen : ConsoleColor.DarkRed);
                fichero.WriteLine("{0:P2}", fms.Matching);
                    ...
                    foreach(ComparatorMatchingScore cms in fms.ComparatorResults){
                        ...
                        while(dms != null){
                            if(level >= dms.DisplayLevel){      
                                ...
                                fichero.WriteLine(string.Format("  Displaying details with a match value > {0:P2}", GetThreshold(dms.DisplayLevel)));
                                fichero.WriteLine();
